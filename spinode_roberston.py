...
def generate_robertson_data_scipy(
    ...
    for i in range(n_series):
        if rand_init and i > 0:
            ...
            y0_perturbed = y0_perturbed / np.sum(y0_perturbed)  # Normalize
        else:
            y0_perturbed = y0
...
class ChuckDataset(Dataset):
    ...
    def __init__(
        ...
        print(f"    ChuckDataset: {self.n_series} series × {self.chuck_per_serie} chunks = {self.total_chuck} total chunks")
        print(f"    Chunk length: {self.chuck_len}, Stride: {self.stride_len}, Samples/chunk: {self.samples_per_chuck}")
...
def save_mlp_params(model: NormalizedMLP, filepath: str):
    """Save MLP parameters to .npz file"""
    params_dict = {
...
def save_crnn_params(model: RobertsonCRNN, filepath: str):
    """Save CRNN parameters to .npz file"""
    params_dict = {
...
def train_stage2(
    ...
    print(f"Interpolated points: {len(ts_interp)}")
    

    dydt_interp = compute_derivatives(ys_interp, ts_interp)
    
    print(f"Training CRNN")
...
def main():
    ...
    print("\n[0] Generating Robertson trajectory data...")
    ...
    # Save all models as .npz files
    print("SAVING MODEL PARAMETERS")
    
    save_mlp_params(mlp_model, 'saved_models/stage1_mlp_params.npz')
    
    save_crnn_params(crnn_stage2, 'saved_models/stage2_crnn_params.npz')
    
    save_crnn_params(crnn_stage3, 'saved_models/stage3_crnn_params.npz')
    ...
    print("  - stage3_crnn_params.npz  (BEST MODEL - use this!)")
    print("  - stage2_crnn_params.npz")
    print("  - stage1_mlp_params.npz")
